...
def test_read_windog_custom_file():
    ...
    df_test = pd.DataFrame(
        [[0, 25, 1013.25, 7, 10, 6], [0, 25, 1013.25, 7, 10, 6], [0, 25, 1013.25, 7, 10, 6]],
...
def test_read_windog_era5_file():
    ...
    df_test = pd.DataFrame(
        [[7.0, 0.0, 25.0, 1013.25], [7.0, 0.0, 25.0, 1013.25], [7.0, 0.0, 25.0, 1013.25]],
...
def test_read_vortex_file():
    ...
    df_test = pd.DataFrame(
        [
            [7.0, 0.0, 25.0, 1.225, 1013.25, 0.1, 50.0],
            [7.0, 0.0, 25.0, 1.225, 1013.25, 0.1, 50.0],
            [7.0, 0.0, 25.0, 1.225, 1013.25, 0.1, 50.0],
        ],
...
df_testCSV = pd.read_csv("tests/df_testCSV.csv", index_col=0, parse_dates=True, infer_datetime_format=True)
...
df_random = pd.DataFrame(np.random.rand(len(df_testCSV), 1), columns=["random test"], index=df_testCSV.index)


def test_create_12x24_12x24Output():
    # test create 12x24 with 12x24 output
    df_test = pd.DataFrame(index=[i for i in range(24)], columns=[i + 1 for i in range(12)])
    for i in range(12):
        df_test[i + 1] = i + 1
...
def test_append_values_differentResolution():
    ...
    assert len(set(tso.append_values(df_testChange, "test", df_testRandom, "random test")["test"])) == len(
        set(df_testRandom["random test"])
    )
...
def test_determineDirectionSector():
    assert (
        tso.determine_direction_sector(df_testCSV, test_parameters)["directionSector"] - df_testCSV["WD SECTOR"]
    ).abs().max() < 1e-5
