...
def create_12x24(df, selected_col, output="column"):
    ...
    df_12x24 = pd.DataFrame(columns=range(1, 13), index=range(24))
    if output == "12x24":
        for month in range(1, 13):
            df_12x24[month] = [
                df[selected_col][(df["month"] == month) & (df["hour"] == hour)].mean() for hour in range(24)
            ]
        ...
        for month in range(1, 13):
            for hour in range(24):
                df["12x24"][(df["month"] == month) & (df["hour"] == hour)] = df[selected_col][
                    (df["month"] == month) & (df["hour"] == hour)
                ].mean()
        df.drop(columns=["month", "hour"], inplace=True)
        return df
    else:
...
def create_with_12x24(df_target, col_target, df_source, col_source):
    ...
    for month in range(1, 13):
        for hour in range(24):
            df_target[col_target][(df_target["month"] == month) & (df_target["hour"] == hour)] = (
                df_12x24[month][hour] * 1
            )
...
def calc_air_density(df, parameters):
    ...
    """
    df["AirDensity"] = (df[parameters["PressureName"][1]] * 100) / (
    ...
    for height in parameters["DesiredHeights"]:  # scale to appropriate value
        scale_factor = (
            parameters["TargetAirDensity"] + (-0.0001181 * (height - parameters["HubHeightTargetAirDensity"]))
        ) / df["AirDensity"].mean()
...
def calc_ti(df, parameters):
    ...
    for height in parameters["DesiredHeights"]:
        df[str(height) + "_TI"] = (df[parameters["HighSensorStd"]] / df[str(height) + "_WS"]) * 100
...
def shear_data(
    ...
    df["shear"] = np.log(df[parameters["HighSensorName"]] / df[parameters["LowSensorName"]]) / np.log(
        parameters["HighSensorHeight"] / parameters["LowSensorHeight"]
    )
    ...
    # shear data
    for height in parameters["DesiredHeights"]:
        df[str(height) + "_WS"] = df[parameters["HighSensorName"]] * pow(
...
def create_openwind_output(df, parameters):
    ...
    for height in parameters["DesiredHeights"]:
        ...
        file_name = os.path.join(
            parameters["OutputLocation"], str(parameters["MastName"]) + "_" + str(height) + "m.csv"
        )
...
def create_masts(parameters):
    ...
    if parameters["TemperatureName"][0] == parameters["MastPrimarySource"]:
        ...
        df = create_with_12x24(df, "Temperature", parameters["InputFiles"][parameters["TemperatureName"][0]], "Temp")
        ...
        df = append_values(df, "Temperature", parameters["InputFiles"][parameters["TemperatureName"][0]], "Temp")
        parameters["TemperatureName"] = (parameters["TemperatureName"][0], "Temperature")
    ...
    if parameters["PressureName"][0] == parameters["MastPrimarySource"]:
        ...
        df = create_with_12x24(df, "Pressure", parameters["InputFiles"][parameters["PressureName"][0]], "Pressure")
        ...
        while flag:
            ...
            for ct, col in enumerate(cols):
                df_tab[col] = [
                    len(
                        df[
                            (df[str(height) + "_WS"] > (i - 0.5))
                            & (df[str(height) + "_WS"] <= (i + 0.5))
                            & (df["directionSector"] == ct)
                        ]
                    )
                    for i in range(36)
                ]
            ...
            if ct == 50:
                raise ValueError(
                    "Taking too long to converge to target wind speed. Please confirm input data are correct."
                )
    return df_tabs
